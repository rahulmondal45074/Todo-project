...
public class TodoController {
    ...
    public String setTodoStatusById(@PathVariable Integer id,@RequestParam boolean flag, @RequestParam String name)
    {
        for(Todo todo : todoList)
        {
            if(todo.getTodoId().equals(id))                                     // 1
            {
                todo.setTodoStatus(flag);                                   ///    4 6
